<x-app-layout>
    <x-slot name="header">
        <x-page-header :title="$user->name" :description="__('Profile, responsibilities, workload, and latest activity for this user.')">
            @can('update', $user)
        ...
        <div class="space-y-6">
            <div class="panel">
                ...
                <dl class="mt-6 space-y-3 text-sm">
                    ...
                    <div class="flex justify-between"><dt class="text-slate-400">{{ __('Task time') }}</dt><dd>{{ \App\Support\Duration::fromMinutes($user->timeEntries->sum('minutes')) }}</dd></div>
                </dl>
            </div>

            <div class="panel">
                <h3 class="text-lg font-semibold text-white">{{ __('Latest activity') }}</h3>
        ...
        <div class="space-y-6">
            <div class="panel">
                ...
                <div class="mt-4 space-y-3">
                    ...
                    @empty
                        <div class="text-sm text-slate-400">{{ __('No assigned tasks.') }}</div>
                    @endforelse
                </div>
            </div>

            <div class="panel">
                <h3 class="text-lg font-semibold text-white">{{ __('Linked projects') }}</h3>
                <div class="mt-4 space-y-3">
                    ...
                    @empty
                        <div class="text-sm text-slate-400">{{ __('No assigned projects yet.') }}</div>
                    @endforelse
                </div>
            </div>
        </div>
